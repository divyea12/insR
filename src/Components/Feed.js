import react, { useState, useEffect } from "react";
import { useNavigate } from 'react-router-dom'
import '../styles/Feed.css'
import { Link } from "react-router-dom"
import axios from 'axios'
import { useAuth } from '../Context/Auth'
import ReactDOM from 'react-dom'
import like from './like.jpeg'
import Like from './Like';
function Feed() {
    const navigate = useNavigate();
    var { user } = useAuth();
    const localuser = JSON.parse(localStorage.getItem("user"));
    if (localuser) user = localuser;
    const { setUser } = useAuth();
    console.log(user);
    const [fileInput, setFileInput] = useState();
    const [selectedFile, setSelectedFile] = useState();
    const [previewSource, setPreviewSource] = useState();
    const [reels, setReels] = useState([]);
    const [like, setLike] = useState(0);
    const [nuser, setNuser] = useState(user);
    function HandleInput(e) {
        const file = e.target.files[0];
        setSelectedFile(file);
        previewFile(file);
    }
    function previewFile(file) {
        const reader = new FileReader();
        reader.readAsDataURL(file);
        reader.onloadend = () => {
            setPreviewSource(reader.result);
        }
        console.log(reader.result);
    }

    function HandleSubmit(e) {
        e.preventDefault();
        // console.log(previewSource);
        if (!previewSource) return;
        uploadImage(previewSource);
    }
    async function uploadImage(base64EncodedVideo) {
        // console.log(typeof(base64EncodedImage));
        let imgstr = JSON.stringify(base64EncodedVideo);
        // console.log((imgstr));    
        // console.log(imgstr===base64EncodedImage);
        try {
            console.log(base64EncodedVideo);
            let res = await axios.post(`https://inst-ulln.onrender.com/upload/${user._id}`, ({ imgstr: base64EncodedVideo }));
            console.log(res.data);
            if (res.data.messsage = "Yayaa") {
                window.alert("uploaded");
                localStorage.setItem('user', JSON.stringify(res.data.user));
                setPreviewSource();
            }
        } catch (err) {
            console.log(err);
        }
    }

    async function logoutHandler() {
        localStorage.removeItem('user');
        navigate('/login');
    }

    function HandleVideoClick(e) {
        e.preventDefault();
        e.target.muted = !e.target.muted;
        e.target.currentTime = 0;
        e.target.play();
    }

    function handleScroll(e) {
        console.log("ended", e);
        let next = ReactDOM.findDOMNode(e.target).parentNode.nextSibling;
        console.log(next);
        if (next) {
            next.scrollIntoView({ behavior: "smooth", block: "end", inline: "nearest" });
            e.target.muted = true;
        }
    }
    function handleControl(e) {
        e.target.muted = false;
        console.log("IN CONTROL");
        e.target.currentTime = 0;
        // e.target.play();
    }

    function handleUp(e) {
        console.log("up");
        let prev = ReactDOM.findDOMNode(e.target).parentNode.previousSibling;
        console.log(prev);
        if (prev) {
            prev.scrollIntoView({ behavior: "smooth", block: "end", inline: "nearest" });
            e.target.muted = true;
        }
        // e.target.pause();
    }

    var posts = [];
    useEffect(() => {
        if (!user) {
            navigate('/login');
        } else {
            // console.log("useefect");
            posts = user.img;
            const req = axios.get("https://inst-ulln.onrender.com/reels")
            req.then(res => { (setReels(res.data.reels)); console.log(res.data.reels); console.log(reels) }).catch(err => console.log(err));
            console.log(reels);
            console.log(posts)
        }
    }, [])



    return (
        <>
            {
                user ?
                    <div>
                        <div className="side">
                            <Link to='/login'><h6 className="h6">Instagram</h6></Link>
                            <div className="prof">
                                <Link to='/profile'><img className="p" src="data:image/jpeg;base64," /></Link>
                            </div>
                        </div>
                        <div className="upload">
                            <form onSubmit={HandleSubmit} encType="multipart/form-data">
                                <input type="file" id="file-upload" name="video" accept='video/*' onChange={HandleInput} value={fileInput} />
                                <label for="file-upload">Upload a Video</label>
                                <input type='submit' id="button" hidden />
                                <label for="button">Submit</label>
                            </form>
                            {
                                previewSource && (
                                    <img src={previewSource} autoplay style={{ height: '10rem', width: '5rem' }} />
                                )
                            }
                        </div>
                        <div className='logout'>
                            <button className="logout-button" onClick={logoutHandler}>Log-Out</button>
                        </div>
                    </div>
                    : <button >Login first</button>
            }
            <div className='reels'>
                {
                    reels.map((reel) => {
                        console.log("helo");
                        console.log(reel.url.includes('image'));
                        // console.log(reel._id);
                        if (reel.url.includes("image")) {
                            return <img className="post" src={reel.url} />
                        } else {
                            return <div className='videoCont'>
                                <video onClick={handleUp} onDoubleClick={handleScroll} onMouseOver={handleControl} onScroll={handleScroll} loop className="post" src={reel.url} autoPlay muted ></video>
                                <Like rid={reel._id} rlike={reel.like} ruser={reel.user} />
                            </div>
                        }
                    })
                }
            </div>
        </>
    )
}
export default Feed;