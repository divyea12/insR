...
function Feed() {
    ...
    async function uploadImage(base64EncodedVideo) {
        ...
        try {
            ...
            let res = await axios.post(`http://localhost:3001/upload/${user._id}`, ({ imgstr: base64EncodedVideo }));
    ...
    return (
        <>
            ...
            <div className='reels'>
                {   
                    user.img.map((imgLink)=>(
                        <img className="post" src={imgLink}/>
                    ))
                }
            </div>
        </>
